...
```SQL
-- Скорее всего це неправильно (скорее всего там, где = ALL), надо бы переделать
-- Но оно действительно должно возвращать 0 строк
SELECT ВОДИТЕЛЬ FROM ЗАКАЗ WHERE "ТОВАР ПО ПРЕЙСКУРАНТУ" = ALL 
(
SELECT DISTINCT "ТОВАР ПО ПРЕЙСКУРАНТУ" FROM ЗАКАЗ as za WHERE NOT EXISTS 
(SELECT "НОМЕР ВЕДОМОСТИ"
FROM ЗАКАЗ WHERE "ЗАКАЗ"."ТОВАР ПО ПРЕЙСКУРАНТУ" = za."ТОВАР ПО ПРЕЙСКУРАНТУ" AND
"ЗАКАЗ"."ЦЕНТР ОБСЛ." IN (
